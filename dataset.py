...
def generate_single_batch(train_input_dir, train_label_dir, batch_paths, crop_flag):
    ...
    for bp in batch_paths:
        ...
        #train_input_image_path = os.path.join(train_input_dir, bp[:-5] + '_q37_' + bp[-5:-4] +'.png')  # q37 png
        img_label = Image.open(train_label_image_path)
        img_input = Image.open(train_input_image_path)
        # randomly crop images to half size.
        if crop_flag:
            width, height = img_input.size
            left = randrange(1,width/2)
            top = randrange(1,height/2)
            right = left + width/2
            bottom = top + height/2
            img_label = img_label.crop((left,top,right,bottom))
